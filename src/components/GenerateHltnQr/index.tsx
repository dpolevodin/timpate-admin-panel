...
import logo from "./assets/hltn-logo.png";
...
export const GenerateHltnQr: FC = () => {
  const [value, setValue] = useState("");
  const qrForDownloadRef = useRef<HTMLDivElement>(null);
  const room = value.split("room=")?.[1];
  ...
  const downloadAsPng = async () => {
    if (qrForDownloadRef.current) {
      try {
        ...
        link.href = image;
        link.download = `tipmate_qr_hilton_room_${room}.png`;
        link.click();
      } catch (error) {
  ...
  return (
    <div className={style.container}>
      <div className={style.text}>
        Paste your link to generate and download the Hltn QR code as png
      </div>

      <input
        onChange={handleChange}
        placeholder={"https://pay.tipmate.me/team/10?room=0001"}
        className={style.input}
        id="hilton-link-input"
      />
